...
import org.apache.rocketmq.proxy.broker.util.PositiveAtomicCounter;
...
public class ProducerManager {
    ...
    private final ConcurrentHashMap<String, String> activeClientIdMap = new ConcurrentHashMap<>();
    ...
    public void scanNotActiveChannel() {
        for (final Entry<String, ConcurrentHashMap<Channel, ClientChannelInfo>> entry : this.groupChannelTable
                .entrySet()) {
    ...
    public synchronized void doChannelCloseEvent(final String remoteAddr, final Channel channel, UnregisterCallback callback) {
        if (channel != null) {
            for (final Entry<String, ConcurrentHashMap<Channel, ClientChannelInfo>> entry : this.groupChannelTable
                    .entrySet()) {
                final String group = entry.getKey();
